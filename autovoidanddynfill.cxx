long dba_dynamic_shape_update_etch(dba_object_handle object_handle, dyn_fill_control_type action, box_type *win)
{
    ...
    // multiuser switch value, to get and set autovoid state.
    ...
    /*
        This check will divert a recursive call to blr_dynamic_shape on
        a shape that is in the process of being blurred.
     */
    ...
    if (action && (action & ~NO_ACTION))
    {
        ...
        {
            /*
                Multiuser only- switch function for letting allegro know when autovoid process
                has started.
             */
        ...
        if (win)
        {
            ...
            /*
                If fill is disabled and no caching, dv_void_window() will NOT
                catch shape being updated. It rejects it cause of same net.
            */
            if (dbg_IsDynamicFillDisabled() && !dbcom_->dyn_fill_cache_ptr)
            {
                ...
                if (allegro_class == CAVITY)
                    call_dv_window = FALSE; // There is no other impacted shapes
            }
            else if ((action & FULL_FILL) && !(action & OOD_GRIDDED) &&
                     !dba_dyn_shape_mode_fast() && !dv_isFrozenShape((shape_type_ptr)object_handle))
            {
                // Note that in fast mode we do not want to delete all the existing voids
                error = _cre_clean_shape(object_handle, NULL);
                dbs_shape_dyn_mask(object_handle, (DYNAMIC_FILL_OOD), TRUE);
            }
            if (call_dv_window)
        ...
        else
        {
            ...
            // in the experimental prototype copy mode we do not want to clean shape here.
...
long _do_dyn_fill(dbptr_type dyn_shape_ptr, int fillMode)
{
    ...
    /*
        We must get list of shapes prior to voiding. More
        shapes will be added to group during this process.
    */
    ...
    {
        dv_grid_regenerating_only_for_serial(TRUE); // SERIAL_OR_PARALLEL
    ...
    for (i = 1; i <= cnt; i++)
    {
        ...
        if (!dbg_IsDynamicFillDisabled())
        {
            ...
            else
            {
                ...
                /*
                    DRC buffer MUST be clean before calling into dynamic shape updates,
                    as that tool relies on DRCs to determine objects needing to be
                    voided around.
                */
                ...
                /*
                    It is not an error for no etch pieces to be left after voiding.
                    But, we no longer need the etch shape, so can get rid of it.
                */
                ...
                adu_cln_net_keep_net_only_for_serial(Prev_keep_net);
            }
            if (UTL_IS_CANCEL)
            {
            ...
            /*
                In case of error set active_shape_ptr back to NULL since
                dba_poly_unfill sets it
            */
            if (didPolyUnfill)
            ...
            // don't consider error set by cancelling out of autovoid an error
...
long dv_autovoid(shape_type *shape_p) // 可能是上层shape类
{
